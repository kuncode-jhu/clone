...
## Python environment setup for ngram language model and LLM rescoring
We use an ngram language model plus rescoring via large language models. The repository ships with support for both [Facebook OPT 6.7b](https://huggingface.co/facebook/opt-6.7b) and Meta's [Llama 3.1 8B/9B](https://huggingface.co/meta-llama) checkpoints. A pretrained 1gram language model is included at [`language_model/pretrained_language_models/openwebtext_1gram_lm_sil`](language_model/pretrained_language_models/openwebtext_1gram_lm_sil). Pretrained 3gram and 5gram language models are available for download [here](https://datadryad.org/dataset/doi:10.5061/dryad.x69p8czpq) (`languageModel.tar.gz` and `languageModel_5gram.tar.gz`). Note that the 3gram model requires ~60GB of RAM, and the 5gram model requires ~300GB of RAM. OPT 6.7b requires a GPU with at least ~12.4 GB of VRAM for full precision inference, while Llama 3.x 8B/9B can be loaded either in full precision on high-memory GPUs or with 4-bit/8-bit quantisation (`--llm_load_in_4bit` / `--llm_load_in_8bit`) to fit on consumer hardware. Switch between backends with the `--llm_backend` flag (e.g., `--llm_backend=llama --llama_model_name meta-llama/Llama-3.1-8B-Instruct`).
